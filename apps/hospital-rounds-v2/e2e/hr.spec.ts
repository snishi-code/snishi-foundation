...
test('フォーマット入力が保存される（write-through）', async ({ page }) => {
  await boot(page);
...
test('患者詳細: 下までスクロールしても下部固定バー全体 (ホーム/ステータス/タグ/QR) が見えて操作できる', async ({
  page,
}) => {
  await boot(page);
  ...
  // バー全体・各ボタンが viewport 内に収まっていること
  await expect(page.locator(ui('detail.actionBar'))).toBeInViewport();
  await expect(page.locator(ui('detail.home'))).toBeInViewport();
  await expect(page.locator(ui('detail.status'))).toBeInViewport();
  await expect(page.locator(ui('detail.tags'))).toBeInViewport();
...
test('ホーム: 患者ボタンの埋め込み QR からその患者の転記 QR が直接開く', async ({ page }) => {
  await boot(page);
  await addPatient(page, 'ホームQR患者');
  // 追加した患者の行 (末尾) の埋め込み QR ボタン
  await page.locator(ui('home.patientQr')).last().click();
  await expect(page.locator(ui('detail.qr.dialog'))).toBeVisible();
  // ホーム画面のまま (詳細へ遷移しない)
  await page.locator(ui('detail.qr.dialog')).getByRole('button', { name: '閉じる' }).click();
  await expect(page.locator(ui('home.grid'))).toBeVisible();
});

test('バイタル: タップ不要の常時入力欄 (BP は "/" と単位が最初から見える)', async ({ page }) => {
  await boot(page);
  await addPatient(page, 'バイタル患者');
  await openPatient(page, 'バイタル患者');
  // BP の分子/分母が常時入力欄として存在し、そのまま入力できる
  const bp1 = page.getByRole('textbox', { name: 'BP 1' });
  await expect(bp1).toBeVisible();
  await bp1.fill('129');
  await page.getByRole('textbox', { name: 'BP 2' }).fill('84');
  // QR プレビューに反映 (write-through 保存)
  await page.locator(ui('detail.qr.show')).click();
  const dialog = page.locator(ui('detail.qr.dialog'));
  await dialog.locator('summary').click();
  await expect(dialog.locator('.qrTextPreview')).toContainText('129/84');
});
